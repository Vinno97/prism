...
glm::mat4 view = glm::lookAt(glm::vec3(5.2f, 0.f, 1.2f),
...
glm::mat4 proj =
    glm::perspective(glm::radians(70.0f), 1920.0f / 1080.0f, 1.0f, 100.0f);
...
void TestRenderer::init() {
  ...
  // Shaders have been uploaded to the GPU and no longer need to be held in
  // memory
  delete vertexShader;
  ...
  // Colours for square
  float colours[] = {
      0.8f, 0.2f, 0.5f, 0.5f, 0.5f, 0.5f, 0.5f, 0.1f, 0.9f, 0.9f, 0.5f, 0.2f,
  };
  ...
  VertexBuffer* colourBuffer =
      renderDevice->createVertexBuffer(sizeof(colours), colours);
