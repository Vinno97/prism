...
namespace ECS
{
	namespace Systems
	{
		...
		EnemyPathFindingSystem::EnemyPathFindingSystem(EntityManager& entityManager, float enemySpeed) : System(
			entityManager)
		{
			...
			if (players.size() > 0)
			{
				...
				for (auto entity : entityManager->getAllEntitiesWithComponent<EnemyComponent>())
				{
						...
						entityManager->hasComponent<VelocityComponent>(entity.id))
					{
						...
						Math::Vector3f vector = Math::Vector3f(playerPosition->x - enemyPosition->x,
						                                       playerPosition->y - enemyPosition->y, 0);
						vector.normalize();
