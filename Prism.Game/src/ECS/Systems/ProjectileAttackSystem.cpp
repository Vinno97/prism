	...
	namespace Systems
	{
		...
		void ProjectileAttackSystem::update(Context& context)
		{
			...
			for (auto entity : entityManager->getAllEntitiesWithComponent<ProjectileAttackComponent>())
			{
				...
				if (entityManager->getComponent<ProjectileAttackComponent>(entity.id) && boundingBoxComponent->
					didCollide)
				{
					...
					for (auto collider : vector)
					{
						if (entityManager->hasComponent<EnemyComponent>(collider))
						{
							...
							if (entityManager->hasComponent<HealthComponent>(entity.id) && entityManager->hasComponent<
								HealthComponent>(collider))
							{
