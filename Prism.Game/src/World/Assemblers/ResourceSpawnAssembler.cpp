...
ResourceSpawnAssembler::ResourceSpawnAssembler()
{
...
void ResourceSpawnAssembler::assemble(int entity, const WorldObject& worldObject,
                                      ECS::EntityManager& entityManager) const
{
...
void ResourceSpawnAssembler::disassemble(int entity, WorldObject& worldObject,
                                         const ECS::EntityManager& entityManager) const
{
	if (entityManager.hasComponent<ECS::Components::ResourceSpawnComponent>(entity))
	{
		...
		worldObject.additionalProperties["value"] = ObjectProperties(
			"value", std::to_string(resouceSpawnComponent->value));
		worldObject.additionalProperties["gatherRate"] = ObjectProperties(
			"gatherRate", std::to_string(resouceSpawnComponent->SpawnTimer));
	}
}
