        ...
        for (int r{0}; r < ROWS; ++r) {
            for (int c = 0; c < COLUMNS; ++c) {
                ...
                if (levels.size() > levelIndex) {
                    ...
                    if (isSavable) {
                        menuBuilder.addControl(x, y, width, height, LEVEL_SLOT_EMPTY, [&, levelIndex]() {
                            if (context.stateMachine->hasState<PrismGame>()) {
                                context.stateMachine->getState<PrismGame>()->Save(
                                        fmt::format("{}/Save {}{}", SAVES, levelIndex + 1, LEVEL_EXTENSION));
                                context.stateMachine->setState<PauseState>();
                            }
                        });
                        ...
                        menuBuilder.addControl(x + .025f, y + height / 4 + .09f, width - 0.05f, .11,
                                               Sprites::WHITE_BORDERED);
                        menuBuilder.addTextControl(x + .05f, y + height / 2, 0.001, Math::Vector3f(0, 0, 0), "Locked");
                    }
                }
